...
def get_price_data(crypto, fiat):
    ...
    df_price = read_from_db(query_price)

    if fiat in EXCHANGE_RATE_SYMBOLS.split(","):
        df_price["price"] = df_price["price"] * df_price[f"{fiat.lower()}_rate"].astype(
            float
        )
...
def get_market_cap_data(crypto, fiat):
    ...
    if fiat in EXCHANGE_RATE_SYMBOLS.split(","):
        df_mcap["market_cap"] = df_mcap["market_cap"] * df_mcap[
            f"{fiat.lower()}_rate"
        ].astype(float)
        df_mcap["fully_diluted_market_cap"] = df_mcap[
            "fully_diluted_market_cap"
        ] * df_mcap[f"{fiat.lower()}_rate"].astype(float)
...
def get_volume_data(crypto, fiat):
    ...
    if fiat in EXCHANGE_RATE_SYMBOLS.split(","):
        df_volume["volume_24h"] = df_volume["volume_24h"] * df_volume[
            f"{fiat.lower()}_rate"
        ].astype(float)
...
def get_general_data(crypto):
    ...
    df_general = read_from_db(query_volume)
...
def layout():
    ...
    with col1:
        selected_crypto = st.selectbox(
            label="Select crypto currency:", options=options_crypto
        )
    with col2:
        selected_currency = st.selectbox(
            label="Select fiat currency:", options=options_currency
        )
    ...
    with gcol2:
        st.metric(
            ...
            value=format_large_numbers(df_gen["circulating_supply"].iloc[0]),
            border=True,
    ...
    with gcol3:
        st.metric(
            "Total Supply (#coins)",
            value=format_large_numbers(df_gen["total_supply"].iloc[0]),
            border=True,
    ...
    fig_volume = px.line(
        df_volume, x="last_updated", y=volume_field, labels=labels_volume
    )
